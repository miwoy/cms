...
const qcloudProxyLogin = require("./qcloudProxyLogin")
...
class WechatApi {
  ...
  async decryptUserInfoData(sessionKey, encryptedData, iv) {
    ...
    try {
      ...
      if (userInfo.watermark.appid !== conf.weixin.appid) {
  ...
  createUnifiedOrder(payInfo) {
    ...
    const weixinpay = new WeiXinPay({
      appid: conf.weixin.appid, // 微信小程序appid
      openid: payInfo.openid, // 用户openid
      mch_id: conf.weixin.mch_id, // 商户帐号ID
      partner_key: conf.weixin.partner_key // 秘钥
    })
    return new Promise((resolve, reject) => {
      weixinpay.createUnifiedOrder({
        ...
        notify_url: conf.weixin.notify_url,
        trade_type: 'JSAPI'
      }, (res) => {
        if (res.return_code === 'SUCCESS' && res.result_code === 'SUCCESS') {
          ...
          const paramStr = `appId=${returnParams.appid}&nonceStr=${returnParams.nonceStr}&package=${returnParams.package}&signType=${returnParams.signType}&timeStamp=${returnParams.timeStamp}&key=` + conf.weixin.partner_key
   ...
   */
  signQuery(queryStr) {
    queryStr = queryStr + '&key=' + conf.weixin.partner_key
    const md5 = require('md5')
